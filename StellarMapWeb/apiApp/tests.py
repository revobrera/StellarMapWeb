...
class CheckUrlViewTestCase(TestCase):
    def test_check_url(self):
        ...
        response = self.client.get(reverse('apiApp:check_url', kwargs={'url': 'http://revobrera.pythonanywhere.com'}))

        # Check that the response is 200 (OK)
        self.assertEqual(response.status_code, 200)

        # Check that the response contains the expected data
        self.assertEqual(response.data, {'url': 'http://revobrera.pythonanywhere.com', 'status': 'OK'})
